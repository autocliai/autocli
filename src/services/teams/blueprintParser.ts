...
export function parseBlueprint(content: string): BlueprintResult {
  ...
  // Only parse metadata from lines before the first agent section
  for (const line of lines) {
    if (/^##\s+Agent:\s*/i.test(line)) break
  ...
  const agents: BlueprintAgent[] = agentSections.map(parseAgentSection)

  return { teamName, goal, schedule, workingDir, agents }
}

function parseAgentSection(section: string): BlueprintAgent {
  ...
  for (const line of lines) {
    ...
    if (providerMatch) { provider = providerMatch[1].trim(); continue }
  ...
  const instructions = extractSubsection(section, 'AGENT.md')
  const soul = extractSubsection(section, 'SOUL.md')
  const memory = extractSubsection(section, 'MEMORY.md')

  return { name, type, model, provider, task, instructions, soul, memory }
}
